...
int setup_handler(void (*f)(int)){
	struct itimerval *tin = (struct itimerval*)malloc(sizeof(struct itimerval));
	struct sigaction sa;
	
	
	
	tin->it_interval.tv_sec = 1;
	tin->it_interval.tv_usec = 0;
	tin->it_value.tv_sec = 1;
	tin->it_value.tv_usec = 0;	
	...
	setitimer(ITIMER_VIRTUAL, tin, NULL);
	return 0;
}
